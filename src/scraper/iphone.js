...
import { REGIONS, REFERENCE_REGION, DEFAULT_IPHONE_MODELS, REQUEST_DELAY } from '../config';
import { standardizeProductName } from '../processor/standardize';
...
export async function getAvailableModels(regionCode = '') {
  try {
    const baseUrl = `https://www.apple.com/${regionCode}/iphone/`;
    const response = await fetch(baseUrl);
    const html = await response.text();
    
    // 使用正則表達式提取型號
    const modelRegex = /\/shop\/buy-iphone\/([\w-]+)/g;
    const matches = [...html.matchAll(modelRegex)];
    const models = [...new Set(matches.map(match => match[1]))];
    
    if (models.length === 0) {
      console.log(`No iPhone models found for region ${regionCode}, using defaults.`);
      // 回退到默認型號
    ...
    console.error('Error getting available iPhone models:', error);
    // 回退到默認型號
    return DEFAULT_IPHONE_MODELS;
  }
}
      ...
      if (metricsData.data && metricsData.data.products && Array.isArray(metricsData.data.products)) {
        ...
        const products = metricsData.data.products.map(product => {
          return {
            ...
            standardName: standardizeProductName(product.name || '')
          };
        });
        ...
        if (productsData && Array.isArray(productsData)) {
          const products = productsData.map(product => {
            ...
            return {
              ...
              region: REGIONS[regionCode][0],
              standardName: standardizeProductName(name)
            };
          });
...
export async function getAllProducts() {
  ...
  for (const regionCode of Object.keys(REGIONS)) {
    for (const model of allModels) {
      try {
        const url = `https://www.apple.com/${regionCode}/shop/buy-iphone/${model}`;
